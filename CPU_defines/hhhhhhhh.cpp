...
enum OPERATIONS_CODES {
    ...
    OPERATION_CODE_RECURSIVE_RET = 42,
...
struct Stack_t {
    int canary_left[1] = { CANARY };
    Elem_t* data;
    size_t size_stack;
    size_t capacity;
    stack_t_status stack_status;
    int stack_hash;
    int canary_right[1] = { CANARY };
};
...
void file_handler(File file) {
    ...
    while(now_command != 0) {
       ...
       switch (now_command) {
# 1 "COMMANDS.H" 1
case COMMAND_HLT: { printf("End of work (hlt!)\n"); print_ram(ram); break; }; break;
...
case COMMAND_PUSH: { Elem_t now_value = 0.0; int flag_of_registers = get_int_from_text(&file, &now_byte); if(flag_of_registers == IS_REGISTER) { int number = get_int_from_text(&file, &now_byte); stack_push(&proc_stack, registers_variables[number]); } else if(flag_of_registers == IS_ELEM_T) { char* push_value = (char*)calloc(MAX_SIZE, sizeof(char)); push_value = get_char_from_text(&file, &now_byte); now_value = string_to_double(push_value); stack_push(&proc_stack, now_value); } else if(flag_of_registers == (IS_RAM | IS_REGISTER)) { int number = get_int_from_text(&file, &now_byte); stack_push(&proc_stack, ram[(int)registers_variables[number]]); } else if(flag_of_registers == (IS_RAM | IS_ELEM_T)) { int number = get_int_from_text(&file, &now_byte); stack_push(&proc_stack, ram[number]); } else { POPADOS(); } }; break;
# 34 "COMMANDS.H"
case COMMAND_POP: { double back_element = 0.0; int flag_of_registers = get_int_from_text(&file, &now_byte); if(flag_of_registers == NOT_ARGS) { back_element = stack_back(&proc_stack); stack_pop(&proc_stack); } else { int number_of_register = get_int_from_text(&file, &now_byte); pop_in_registers(number_of_register, registers_variables, &proc_stack); } }; break;
# 49 "COMMANDS.H"
case COMMAND_ADD: { double last = stack_back(&proc_stack); stack_pop(&proc_stack); double penultimate = stack_back(&proc_stack); stack_pop(&proc_stack); stack_push(&proc_stack, last + penultimate); }; break;
# 60 "COMMANDS.H"
case COMMAND_SUB: { double last = stack_back(&proc_stack); stack_pop(&proc_stack); double penultimate = stack_back(&proc_stack); stack_pop(&proc_stack); stack_push(&proc_stack, penultimate - last); }; break;
# 71 "COMMANDS.H"
case COMMAND_MUL: { double last = stack_back(&proc_stack); stack_pop(&proc_stack); double penultimate = stack_back(&proc_stack); stack_pop(&proc_stack); stack_push(&proc_stack, penultimate * last); }; break;
# 82 "COMMANDS.H"
case COMMAND_DIV: { double last = stack_back(&proc_stack); stack_pop(&proc_stack); double penultimate = stack_back(&proc_stack); stack_pop(&proc_stack); stack_push(&proc_stack, penultimate / last); }; break;
# 93 "COMMANDS.H"
case COMMAND_OUT: { if(proc_stack.size_stack == 0) { printf("Stack is empty\n"); } else { printf("Answer: (size = %d) ", proc_stack.size_stack); printf(identity, stack_back(&proc_stack)); printf("\n\n"); } }; break;
# 105 "COMMANDS.H"
case COMMAND_IN: { scanf(identity, &input_value); stack_push(&proc_stack, input_value); }; break;





case COMMAND_SQRT: { double last = stack_back(&proc_stack); stack_pop(&proc_stack); stack_push(&proc_stack, sqrt(last)); }; break;
...
# 36 "common.cpp"
   "file"
# 36 "common.cpp" 3 4
   , "common.cpp", 36, __extension__ __PRETTY_FUNCTION__))
# 36 "common.cpp"
               ;
...
# 37 "common.cpp"
   "name_file"
# 37 "common.cpp" 3 4
   , "common.cpp", 37, __extension__ __PRETTY_FUNCTION__))
# 37 "common.cpp"
                    ;
...
const struct call_of_dump base_arguments_of_call = {"stack.cpp", -1, " "};
...
int get_hash(Stack_t* node) {
    ...
    hash_st = (hash_st + node->canary_left[0] % HASH_NUMBER) % HASH_NUMBER;
    hash_st = (hash_st + node->canary_right[0] % HASH_NUMBER) % HASH_NUMBER;
...
bool is_canary(Elem_t value) { return (value == CANARY); }
...
void stack_err(Stack_t* node, struct call_of_dump arguments_of_call = base_arguments_of_call) {
    ...
    if(node == nullptr) {
        ...
        stack_dump(node, create_struct("stack.cpp", 178, __FUNCTION__));
    }
    if(node->data == nullptr && node->stack_status != STACK_EMPTY) {
        node->stack_status = STACK_EMPTY;
        stack_dump(node, create_struct("stack.cpp", 182, __FUNCTION__));
    }
    if((node->size_stack) > (node->capacity)) {
        node->stack_status = STACK_OVERFLOW;
        stack_dump(node, create_struct("stack.cpp", 186, __FUNCTION__));
    }
    if(node->size_stack < 0) {
        node->stack_status = STACK_BAD_SIZE;
        stack_dump(node, create_struct("stack.cpp", 190, __FUNCTION__));
    }
    if(node->capacity < 0) {
        node->stack_status = STACK_BAD_CAPACITY;
        stack_dump(node, create_struct("stack.cpp", 194, __FUNCTION__));
    }

    if(node != nullptr && node->data != nullptr && get_hash(node) != node->stack_hash) { node->stack_status = STACK_BAD_HASH; stack_dump(node, create_struct("stack.cpp", 200, __FUNCTION__)); }
    ...
    if(node->canary_left[0] != CANARY || node->canary_right[0] != CANARY) { node->stack_status = STACK_BAD_CANARY; stack_dump(node, create_struct("stack.cpp", 207, __FUNCTION__)); }
...
# 239 "stack.cpp"
   "node != nullptr"
# 239 "stack.cpp" 3 4
   , "stack.cpp", 239, __extension__ __PRETTY_FUNCTION__))
# 239 "stack.cpp"
                          ;
...
# 277 "stack.cpp" 3 4
   (static_cast <bool> (
# 277 "stack.cpp"
   node->capacity >= stack_size(node)
# 277 "stack.cpp" 3 4
   ) ? void (0) : __assert_fail (
# 277 "stack.cpp"
   "node->capacity >= stack_size(node)"
# 277 "stack.cpp" 3 4
   , "stack.cpp", 277, __extension__ __PRETTY_FUNCTION__))
# 277 "stack.cpp"
                                             ;
...
# 283 "stack.cpp"
   "new_data != nullptr"
# 283 "stack.cpp" 3 4
   , "stack.cpp", 283, __extension__ __PRETTY_FUNCTION__))
# 283 "stack.cpp"
                              ;
    node->data = new_data;
