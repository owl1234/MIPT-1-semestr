...
LIST_STATUSES list_insert_after(List* my_list, const size_t physical_position, const Elem_type value) {
    ...
    return list_insert_before(my_list, my_list->data[physical_position].next, value);
}
...
LIST_STATUSES list_insert_first_element(List* my_list, const size_t temporary_free,  const Elem_type value) {
    ...
    my_list->data[my_list->head].next    = my_list->head; //->fictive;
    my_list->data[my_list->head].prev    = my_list->tail;
...
void warning(List* my_list, struct call_of_dump arguments_of_call) {
